...
object Main extends App {
  override def run(args: List[String]): URIO[ZEnv, ExitCode] =
    Cli.parse(args).flatMap { config =>
      dependencies(config).build.use { env => transport.downloadApartments().provide(env) }
    }.exitCode
  ...
  def dependencies(config: TransportAppConfig): ZLayer[Any, Nothing, DataTransport] = {
    ...
    val fs = if(config.debug) appConfig >>> FS.local else {
    ...
    val clientBackend: ZLayer[Any, Nothing, Has[OnlinerBackend]] = ZLayer.fromManaged(AsyncHttpClientZioBackend.managed()).orDie
